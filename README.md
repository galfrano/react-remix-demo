...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create node-api-4e37
  fly apps create node-api-4e37-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app node-api-4e37
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app node-api-4e37-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'node-api-4e37-staging' does not match app name in config file 'node-api-4e37'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name node-api-4e37-db
  fly postgres attach --app node-api-4e37 node-api-4e37-db

  fly postgres create --name node-api-4e37-staging-db
  fly postgres attach --app node-api-4e37-staging node-api-4e37-staging-db
  ```
